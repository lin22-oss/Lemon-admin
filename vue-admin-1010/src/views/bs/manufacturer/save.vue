    ...
    厂商表单
     <el-form label-width="120px">
      ...
      <el-form-item label="厂商排序">
        <el-input-number v-model="manufacturer.sort" controls-position="right" min="0"/>
      </el-form-item>
...
export default {
  ...
  methods:{
    ...
    //根据厂商id查询的方法
    getInfo(id) {
      manufacturer.getManufacturerInfo(id)
        .then(response => {
    ...
    //修改厂商的方法
    updateManufacturer() {
      manufacturer.updateManufacturerInfo(this.manufacturer)
        .then(response => {
          //提示信息
          this.$message({
              type: 'success',
    ...
    //添加厂商的方法
    saveManufacturer() {
      manufacturer.addManufacturer(this.manufacturer)
        .then(response => {//添加成功
          //提示信息
          this.$message({
              type: 'success',
